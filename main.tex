\documentclass{book}

\usepackage[spanish]{babel}
\usepackage[a4paper,top=2cm,bottom=2cm,left=2.5cm,right=2.5cm,marginparwidth=1.75cm]{geometry}

%%-----------------------------------------------
%% Paquetes necesarios
%%-----------------------------------------------
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{mathtools}
\usepackage{graphicx}
\usepackage{hyperref}
\usepackage{tcolorbox}
\usepackage{listings}
\usepackage{xcolor}
\usepackage{eurosym}
\usepackage{fancyhdr}
\usepackage{float}
\usepackage{subcaption}
\usepackage[utf8]{inputenc}
\usepackage{pdfpages}
\usepackage{tabularx}
\usepackage{array}


\input{datos.tex}



%%-----------------------------------------------
%% Definicion de entornos
%%-----------------------------------------------
\newtheorem{remark}{Observación}
\newtheorem{proposition}{Proposición}
\newtheorem{notation}{Notación}

\setcounter{secnumdepth}{3}
\setcounter{tocdepth}{3}


%%-----------------------------------------------
%% Cabeceras y pies de página:
%%-----------------------------------------------
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LO]{\leftmark}
\fancyhead[RE]{\rightmark}
\setlength{\headheight}{1.5\headheight}
\cfoot{\thepage}


%%-----------------------------------------------
%% Páginas en blanco sin cabecera:
%%-----------------------------------------------
\makeatletter%
\addto\shorthandsspanish{\let\esperiod\es@period@code}%
\def\clearpage{%
  \ifvmode%
    \ifnum\@dbltopnum=\m@ne%
      \ifdim\pagetotal<\topskip%
        \hbox{}%
      \fi%
    \fi%
  \fi%
  \newpage%
  \thispagestyle{empty}%
  \write\m@ne{}%
  \vbox{}%
  \penalty-\@Mi%
}%
\makeatother



\begin{document}

  \begin{titlepage}
      \centering
      \vspace{3cm}
      {\includegraphics[width=0.9\textwidth]{newplot.png}\par}
      \vspace{5cm}
      {\scshape\Huge \Titulo\par}
      \vspace{2cm}
      {\scshape\LARGE Apuntes-esquema \par}
      \vfill
      {\Large Autor:  \par}
      {\Large \NombreAutor\par}
      \vfill
      {\Large \today \par}
  \end{titlepage}


  \clearpage%
  \tableofcontents
  \setcounter{page}{1}
  \pagenumbering{arabic}


  \clearpage%
  \part{Fundamentos básicos de derivados, riesgo y rentabilidad}
  \chapter{Introducción}
  \input{1_Conceptos_basicos}

  \clearpage%
  \chapter{Productos financieros básicos}
  \input{2_Opciones}
  \newpage
  \input{11_Prods_renta_fija.tex}
  \newpage
  \input{12_swaps.tex}

  \clearpage%
  \chapter{Aletoriedad}
  \input{3_ALetoriedad}

  \clearpage%
  \chapter{Modelación básica de opciones}
  \input{4_Black_Scholes}
  \newpage
  \input{5_EDPs}
  \newpage
  \input{6_Sols_deltas}
  \newpage
  \input{8_Generalizaciones}
  \newpage
  \input{7_Volatilidad}
  \newpage
  \input{10_Cobertura}
  \newpage
  \input{9_Ops_Americanas}
  \newpage
  \input{13_multi_asset.tex}
  
  \clearpage%
  \chapter{Valoración del riesgo}
  \input{14_Gestion_carteras.tex}
  \newpage
  \input{15_VaR.tex}

  \clearpage%
  \chapter{Predicción del mercado}
  \input{16_Prediccion_mercado.tex}




  \clearpage%
  \appendix
  \chapter{Apéndices Introducción}
  \input{Apendices/Apendice Ito}
  \input{Apendices/Apendice generalizaciones}
  \input{Apendices/Apendice Make Your Mind Up}
  \input{Apendices/Apendice var cob vol imp.tex}


  \chapter{Apéndices extra}
  \input{Apendices/Black_Jack.tex}

\end{document}
